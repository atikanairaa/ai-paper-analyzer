...
export default function Dashboard() {
  ...
  return (
    <AppLayout  defaultRole="admin">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                  {failedJobs.length === 0 && (
                    <tr><td colSpan={4} className="px-6 py-10 text-center text-slate-400">Tidak ada tugas gagal saat ini 🎉</td></tr>
                  )}
                  {failedJobs.map(job => (
                    <tr key={job.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-[11px] text-slate-400 mt-1">Durasi: {job.duration_seconds}d · Retry: {job.retry_count}/3</div>
                      </td>
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 bg-slate-900 text-white px-5 py-3 rounded-xl shadow-2xl flex items-center space-x-3 z-50">
