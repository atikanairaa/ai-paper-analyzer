...
export default function ReviewDetail() {
    ...
    if (!paper) {
        return <AppLayout defaultRole="reviewer"><div className="p-8 text-center text-stone-600">Paper tidak ditemukan.</div></AppLayout>;
    }
    ...
    const handleReviewSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (recommendation === 'ACCEPT') confirmBtnClass = 'bg-emerald-600 hover:bg-emerald-700 text-white';
        if (recommendation === 'MINOR_REVISION') confirmBtnClass = 'bg-amber-600 hover:bg-amber-700 text-white';
        if (recommendation === 'MAJOR_REVISION') confirmBtnClass = 'bg-orange-700 hover:bg-orange-800 text-white';
        if (recommendation === 'REJECT') confirmBtnClass = 'bg-rose-700 hover:bg-rose-800 text-white';
    ...
    const analysis = Array.isArray(paper?.analyses) ? paper.analyses[0] : (paper?.analyses || {});
    ...
    const existingReview = Array.isArray(paper?.reviews) ? paper.reviews[0] : (paper?.reviews || null);
    const currentScore = existingReview?.score || paper?.scores?.overall_score || '';
    ...
    useEffect(() => {
        ...
        fetch(`/papers/${paper.id}/pdf-view`)
            ...
            .then(pdfBlob => {
                ...
                formData.append('file', new File([pdfBlob], `paper_${paper.id}.pdf`, { type: 'application/pdf' }));
                formData.append('watermark_text', 'CONFIDENTIAL — FOR PEER REVIEW ONLY');
            ...
            <div className="max-w-[1440px] mx-auto p-4 md:p-6 lg:p-8 h-[calc(100vh-64px)] flex flex-col">
                ...
                <div className="mb-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <h1 className="text-xl font-bold text-stone-900 truncate max-w-xl">{paper?.title || 'Judul Tidak Tersedia'}</h1>
                        <Badge color={paper?.submission_status === 'REVIEWED' ? 'green' : 'yellow'}>
                            {paper?.submission_status === 'REVIEWED' ? 'Selesai Direview' : 'Butuh Review'}
                        </Badge>
                    </div>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-6 flex-1 h-full min-h-0">
                    ...
                    <div className="lg:w-[40%] flex flex-col h-full bg-white border border-[#e8e4dc] rounded-2xl shadow-sm overflow-hidden">
                        <div className="bg-stone-50 p-4 border-b border-[#e8e4dc]">
                            <h2 className="font-bold text-stone-900">Panel Evaluasi Reviewer</h2>
                            <p className="text-xs text-stone-500 mt-0.5">Penilaian bersifat rahasia dan akan dikirim ke editor</p>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-[#faf8f5]">
                            ...
                            <div className="bg-white p-4 rounded-xl border border-[#e8e4dc] shadow-sm">
                                <h3 className="text-sm font-bold text-stone-900 mb-2 uppercase tracking-wider">Ringkasan AI</h3>
                                <p className="text-sm text-stone-700 mb-3 leading-relaxed">{paper?.abstract || "Tidak ada abstrak."}</p>
                                
                                {strengths.length > 0 && (
                                    <div className="mb-3">
                                        <p className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Kekuatan</p>
                                ...
                                {weaknesses.length > 0 && (
                                    <div className="mb-3">
                                        <p className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Kelemahan</p>
                                        <ul className="list-disc pl-5 text-sm text-stone-700 space-y-1">
                                            {weaknesses.slice(0, 3).map((s: string, i: number) => <li key={i}>{s}</li>)}
                                        </ul>
                                    </div>
                                )}
                            </div>
                            ...
                            {paper.submission_status === 'REVIEWED' ? (
                                <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-xl text-center shadow-sm">
                                    ...
                                    <h3 className="font-bold text-emerald-900 mb-1">Review Selesai</h3>
                                    <p className="text-sm text-emerald-700">Anda telah mensubmit evaluasi untuk paper ini.</p>
                                </div>
                            ) : (
                                <form onSubmit={handleReviewSubmit} className="bg-white p-5 rounded-xl border border-[#e8e4dc] shadow-sm space-y-5">
                                    <h3 className="text-sm font-bold text-stone-900 uppercase tracking-wider mb-4 border-b border-[#e8e4dc] pb-2">Formulir Keputusan</h3>
                                    
                                    <div>
                                        <label className="block text-xs font-bold text-stone-600 uppercase tracking-wider mb-2">Skor Evaluasi (0 - 100)</label>
                                        <input
                                            required
                                            name="score"
                                            type="number"
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-stone-600 uppercase tracking-wider mb-2">Catatan Evaluasi Mendalam</label>
                                        <textarea
                                            required
                                            name="comments"
                                            rows={6}
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-stone-600 uppercase tracking-wider mb-3">Vonis Keputusan Akhir</label>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            <button 
                                                type="submit" 
                                                onClick={(e) => { (e.currentTarget.form as any).recommendation.value = 'ACCEPT'; }}
                                            ...
                                            <button 
                                                type="submit"
                                                onClick={(e) => { (e.currentTarget.form as any).recommendation.value = 'MINOR_REVISION'; }}
                                            ...
                                            <button 
                                                type="submit"
                                                onClick={(e) => { (e.currentTarget.form as any).recommendation.value = 'MAJOR_REVISION'; }}
                                            ...
                                            <button 
                                                type="submit"
                                                onClick={(e) => { (e.currentTarget.form as any).recommendation.value = 'REJECT'; }}
